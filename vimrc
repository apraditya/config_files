...
if executable('rg')
  ...
  command! -bang -nargs=* Find call fzf#vim#grep('rg --column --line-number --no-heading --fixed-strings --ignore-case --hidden --follow --glob "!.git/*" --color "always" '.shellescape(<q-args>).'| tr -d "\018"', 1, <bang>0)
endif
...
augroup vimrc-html
  autocmd!
  autocmd FileType html set tabstop=2|set shiftwidth=2|set expandtab softtabstop=2
augroup END
